...
jupyterlab: up
	...
	@echo "    To access jupyterlab, access the URL showing later as usual \n"
	docker exec -it budoka-container jupyter lab --ip='0.0.0.0' --allow-root --no-browser --NotebookApp.token=''
...
both: up
	...
	docker exec -it budoka-container bash -c "jupyter lab --ip='0.0.0.0' --allow-root --no-browser --NotebookApp.token='' & mlflow ui --host 0.0.0.0"
